 ...
 **/
void client::init_connection(char* ip_addr, fd_set* main_fd, fd_set* cli_fds, int* sock_fd, SA_in* serv_addr)
{
    // Establish socket
 ...
 **/
void client::run_client(fd_set* cli_fds, fd_set* main_fd, int* max_fd, int* sock_fd, char* name)
{
    *max_fd = *sock_fd;
 ...
 * clients).
 **/
void client::send_msg(char* name, int sock_fd)
{
 ...
 **/
void client::recv_msg(int sock_fd)
    {
...
int main(int argc, char** argv)
{
	...
	client_object.init_connection("127.0.0.1", &main_fd, &cli_fds, &sock_fd, &serv_addr);
	...
	client_object.run_client(&cli_fds, &main_fd, &max_fd, &sock_fd, name);
